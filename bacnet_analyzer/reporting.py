...
def generate_devices_report(device_cache: Dict[str, DeviceInfo]) -> List[str]:
    ...
    id_based_devices = {k: v for k, v in device_cache.items() if k.startswith("device:")}
    address_based_devices = {k: v for k, v in device_cache.items() if not k.startswith("device:")}
