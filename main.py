...
from typing import Dict, Set, Counter as CounterType, Optional, Tuple, List, Any
...
class BACnetPcapAnalyzer:
    ...
    def _process_packet(self, frame):
        ...
        if hasattr(frame, 'udp') and frame.udp and frame.udp.source_port == 47808:
            ...
            if ip_src:
                self._update_stats(ip_src, "BACnet/IP", ip_dst, is_broadcast=ip_dst.endswith("255.255.255"))
            ...
            if hasattr(frame, 'bvll') and frame.bvll:
                ...
                if bvll_type == 'ForwardedNPDU':
                    if frame.npci and hasattr(frame.npci, 'npduSADR'):
                        ...
                        if hasattr(npdu_sadr, 'addrAddr'):
                            ...
                            if mac:
                                ...
                                self._update_stats(actual_source, "ForwardedNPDU", ip_dst)
                    if ip_src:
                    ...
                    if hasattr(frame.bvll, 'bvlciAddress') and frame.bvll.bvlciAddress:
                        ...
                        if self.debug:
                            print(f"Processing forwarded NPDU from router {router_addr}")
        ...
        if hasattr(bvll, 'npdu') and bvll.npdu:
            ...
            if hasattr(npdu, 'npduSADR') and npdu.npduSADR:
                ...
                if hasattr(sadr, 'addrNet'):
                    ...
                    if self.debug:
                        print(f"Found BACnet source network: {network}, type: {addr_type}")
                ...
                if hasattr(sadr, 'addrAddr'):
                    ...
                    mac = ''.join(f'{b:02x}' for b in mac_bytes) if mac_bytes else None
                    if self.debug:
                        print(f"Found BACnet source MAC: {mac}, raw bytes: {sadr.addrAddr!r}")
            ...
            if hasattr(npdu, 'apdu') and npdu.apdu:
                ...
                if apdu_type == 'IAmRequest' and hasattr(apdu, 'iAmDeviceIdentifier'):
                    ...
                    self._process_i_am_request(apdu, original_src, pkt_time, npdu, is_forwarded=True)
                    
                    if self.debug:
                        print(f"Found I-Am in forwarded packet NPDU (legacy path), via router {original_src}")
        ...
        if hasattr(bvll, 'pduData') and bvll.pduData:
            try:
                ...
                if decoded_pkt:
                    ...
                    if network is None and hasattr(decoded_pkt, 'npdu') and decoded_pkt.npdu:
                    ...
                    if hasattr(decoded_pkt, 'apdu'):
                        ...
                        if not source_stats_updated and network is not None and mac is not None:
                            actual_source = f"{network}:{mac}"
                            self._update_stats(actual_source, apdu_type, destination)
                            source_stats_updated = True
                        ...
                        if hasattr(apdu, 'iAmDeviceIdentifier'):
                            self._process_i_am_request(apdu, original_src, pkt_time, decoded_pkt.npdu if hasattr(decoded_pkt, 'npdu') else None, is_forwarded=True)
                            
                            if self.debug:
                                print(f"Found I-Am in decoded packet, via router {original_src}")
    ...
    def _process_apdu(self, apdu, source, destination, pkt_time, npdu=None):
        ...
        if apdu_type == 'IAmRequest' and hasattr(apdu, 'iAmDeviceIdentifier'):
            ...
            if self.debug:
                print(f"Found I-Am message from {source}, device ID: {device_id} (processed via legacy path)")
    ...
    def _process_i_am_request_new(self, frame, is_forwarded=False):
        ...
        if hasattr(frame, 'apdu') and frame.apdu:
            apdu = frame.apdu
        elif hasattr(frame, 'bvll') and frame.bvll and hasattr(frame.bvll, 'npdu') and frame.bvll.npdu:
        ...
        try:
            ...
            if self.debug:
                print(f"Processing device ID info: {device_id_info} (type: {type(device_id_info).__name__})")
            ...
            if isinstance(device_id_info, tuple) and len(device_id_info) == 2:
                ...
                if ',' in device_id_str:
                    ...
                    if self.debug:
                        print(f"Extracted device ID {device_id} from string representation")
        except Exception as e:
            if self.debug:
        ...
        #     # Extract address details from the remoteStation
            ...
            if hasattr(sadr, 'addrNet'):
                network = sadr.addrNet
                if self.debug:
                    print(f"I-Am processing: Found source NPDU network: {network}, type: {addr_type}")
            ...
            if hasattr(sadr, 'addrAddr'):
                ...
                mac = ''.join(f'{b:02x}' for b in mac_bytes) if mac_bytes else None
                if self.debug:
                    print(f"I-Am processing: Found source NPDU MAC: {mac}, raw bytes: {sadr.addrAddr!r}")
                    print(f"I-Am processing: BACnet address is {network}:{mac}")
                    print(f"I-Am processing: Full NPDU source address info: {sadr.__dict__ if hasattr(sadr, '__dict__') else 'No __dict__'}")
        ...
        if network is not None and mac is not None:
            ...
            if self.debug:
                print(f"DEBUG: Using BACnet network address {device_key} for device {device_id}")
        else:
        ...
        if self.debug:
            ...
            if mac is None:
                ...
                print(f"Complete BACnet address {network}:{mac} found for device {device_id}")
    ...
    def _process_i_am_request(self, apdu, source_addr, pkt_time, npdu=None, is_forwarded=False):
        ...
        if network is not None and mac is not None:
            ...
            device_key = f"{network}:{mac}"
            key_type = "BACnet network address"
            print(f"DEBUG: Using BACnet network address {device_key} for device {device_id}") if self.debug else None
        else:
            # Otherwise use local network (0) with source IP address
            device_key = f"0:{source_addr}"
            key_type = "IP address"
            print(f"DEBUG: Using IP address {device_key} for device {device_id}") if self.debug else None
    ...
    def _update_stats(self, src_addr: str, msg_type: str, dest_addr: Optional[str] = None, 
                      is_routed: bool = None, is_broadcast: bool = None, pkt_time: float = 0):
        ...
        if is_routed is None:
            is_routed = msg_type in ('ForwardedNPDU', 'RouterToNetwork', 'NetworkToRouter')
        ...
        if is_broadcast is None and dest_addr:
            is_broadcast = ('255.255.255.255' in dest_addr or 
                           '*.255.255.255' in dest_addr or 
                           '*' in dest_addr)
    ...
    def _debug_frame(self, frame):
        ...
        if hasattr(frame, 'ipv4') and frame.ipv4:
            print(f"IPv4: src={frame.ipv4.source_address}, dst={frame.ipv4.destination_address}")
        
        if hasattr(frame, 'udp') and frame.udp:
            print(f"UDP: src_port={frame.udp.source_port}, dst_port={frame.udp.destination_port}")
        
        if hasattr(frame, 'bvlci') and frame.bvlci:
            print(f"BVLCI: type={frame.bvlci.bvlciType}, function={frame.bvlci.bvlciFunction}")
        ...
        if hasattr(frame, 'npdu') and frame.npdu:
            ...
            for attr in dir(frame.npdu):
                if not attr.startswith('_') and attr not in ('encode', 'decode', 'copy', 'dict_contents'):
                    try:
    ...
    def print_summary(self):
        ...
        for addr, stats in sorted(self.address_stats.items()):
            ...
            print(f"    Routed: {stats.routed_messages}, Non-Routed: {stats.non_routed_messages}")
            print(f"    Unicast: {stats.unicast_messages}, Broadcast: {stats.broadcast_messages}")
        ...
        if not self.device_cache:
            ...
            id_based_devices = {k: v for k, v in self.device_cache.items() if k.startswith("device:")}
            address_based_devices = {k: v for k, v in self.device_cache.items() if not k.startswith("device:")}
            ...
            if not any_remote_station and address_based_devices:
                ...
                print("  2. Forwarded packets don't contain complete NPDU source address information")
