...
class CoronaMetricsGenerator:
    ...
    def __init__(
        ...
        Args:
            analyzer: The BACnet PCAP analyzer instance containing the processed data
    ...
    def _add_interface_to_graph(
        self, interface_key, interface_info, metrics, capture_device_uri=None
    ):
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python corona_metrics_rdflib.py <pcap_file> <output_ttl_file> [--capture-device=<address>] [--debug]"
        )
        sys.exit(1)
    ...
    for arg in sys.argv[3:]:
        if arg == "--debug":
            debug_mode = True
        elif arg.startswith("--capture-device="):
            capture_device = arg.split("=", 1)[1]

    CoronaMetricsGenerator.process_pcap_and_generate_metrics(
        pcap_file, output_file, capture_device=capture_device, debug=debug_mode
    )
